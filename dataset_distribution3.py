...
def calcHist(imgs):
    ...
    return hist, info
def drawHist_per_dataset(img_paths, prefix, mode='depth', imgs_min=0, imgs_max=0):
    ...
    for idx, path in tqdm(enumerate(img_paths)):
        ...
        if img.min() == 0:
            depth_zero_paths.append(path)
            sem_zero_paths.append(path.replace('/Depth/','/SEM/').replace('.png', '_itr0.png'))
        else:
            depth_non_zero_paths.append(path)
            sem_non_zero_paths.append(path.replace('/Depth/','/SEM/').replace('.png', '_itr0.png'))

    img = cv2.imread(sem_zero_paths[0],0)
    sem_zero_hist = cv2.calcHist([img], [0], None, [256], [0, 256]) / img.size * 100

    img = cv2.imread(depth_zero_paths[0])
    depth_zero_hist = cv2.calcHist([img], [0], None, [256], [0, 256]) / img.size * 100
    if mode == 'depth':
        for i in range(256):
            if depth_zero_hist[i] > 1.0:
                depth_zero_hist[i] = 1.0

    img = cv2.imread(sem_non_zero_paths[0])
    sem_non_zero_hist = cv2.calcHist([img], [0], None, [256], [0, 256]) / img.size * 100

    img = cv2.imread(depth_non_zero_paths[0])
    depth_non_zero_hist = cv2.calcHist([img], [0], None, [256], [0, 256]) / img.size * 100
    ...
    ax0.bar(np.arange(256), sem_zero_hist.squeeze(), color='b')
    ax0.set_xlim([0, 256])
    ax0.set_xticks([i*10 for i in range(26)])
    ax0.set_title('sem_zero_hist')

    ax1 = fig.add_subplot(spec[1])
    ax1.bar(np.arange(256), depth_zero_hist.squeeze(), color='b')
    ax1.set_xlim([0, 256])
    ax1.set_xticks([i*10 for i in range(26)])
    ax1.set_title('depth_zero_hist')

    ax2 = fig.add_subplot(spec[2])
    ax2.bar(np.arange(256), sem_non_zero_hist.squeeze(), color='b')
    ax2.set_xlim([0, 256])
    ax2.set_xticks([i*10 for i in range(26)])
    ax2.set_title('sem_non_zero_hist')

    ax3 = fig.add_subplot(spec[3])
    ax3.bar(np.arange(256), depth_non_zero_hist.squeeze(), color='b')
    ax3.set_xlim([0, 256])
    ax3.set_xticks([i*10 for i in range(26)])
    ax3.set_title('depth_non_zero_hist')
...
if __name__ == '__main__':
    ...
    #
'/data2/samsung/simulation_data/SEM/Case_4/80/20210114_112507_NE142400C_QGLE069_1_S11_M0051-01MS_0.png'