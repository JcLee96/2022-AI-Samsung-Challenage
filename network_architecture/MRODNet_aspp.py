...
class Transition(nn.Module):
    def __init__(self, n_channels, n_output_channels):
        ...
        self.conv1 = nn.Conv2d(n_channels, n_output_channels, kernel_size=3, padding=1, bias=False)
...
class ResidualBlock(nn.Module):
    def __init__(self, n_channels):
        ...
        self.conv2 = nn.Conv2d(n_channels, n_channels, kernel_size=3, groups=int(n_channels / 8), padding=1, bias=False)
        self.conv3 = nn.Conv2d(n_channels, n_channels, kernel_size=3, padding=1, bias=False)
...
class SceneUnderstandingModule(nn.Module):
    def __init__(self, n_channels, scale_factor, device=torch.device("cuda:0"), dilations=[1, 3, 6, 9]):
        ...
        self.aspp1 = nn.Sequential(
            ...
            nn.Conv2d(n_channels, out_n_channels, kernel_size=1, stride=1, padding=0, dilation=dilations[0]),
        ...
        self.aspp2 = nn.Sequential(
            ...
            nn.Conv2d(n_channels, out_n_channels, kernel_size=3, stride=1, padding=dilations[1], dilation=dilations[1]),
            nn.GroupNorm(1, out_n_channels),
            nn.ReLU(inplace=True)
        )
        self.aspp3 = nn.Sequential(
            #nn.Conv3d(n_channels, out_n_channels, kernel_size=3, stride=1, padding=dilations[2], dilation=dilations[2]),
            nn.Conv2d(n_channels, out_n_channels, kernel_size=3, stride=1, padding=dilations[2], dilation=dilations[2]),
            nn.GroupNorm(1, out_n_channels),
            nn.ReLU(inplace=True)
        )
        self.aspp4 = nn.Sequential(
            #nn.Conv3d(n_channels, out_n_channels, kernel_size=3, stride=1, padding=dilations[3], dilation=dilations[3]),
            nn.Conv2d(n_channels, out_n_channels, kernel_size=3, stride=1, padding=dilations[3], dilation=dilations[3]),
            nn.GroupNorm(1, out_n_channels),
            nn.ReLU(inplace=True)
        )
        self.concat_process = nn.Sequential(
            #nn.Conv3d(out_n_channels * 5, n_channels, 1),
            nn.Conv2d(out_n_channels * 5, n_channels, 1),
    ...
    def forward(self, x):
        ...
        global_avg_pool = nn.Sequential(#nn.AdaptiveAvgPool3d((d, 1, 1)),
                                        ...
                                        nn.Conv2d(self.n_channels, self.out_n_channels, 1, stride=1, bias=False),
...
class OrdinalRegressionLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        xo = x
        # P is phase number. In this case, P = 1
        N, C, H, W = x.size()
        x = x.view(N, -1, 1, H, W)
        N, C, P, H, W = x.size()
        ord_num = C // 2
        ...
        OA = A
        OB = B
        A = A.view(N, 1, ord_num * P * H * W)
        B = B.view(N, 1, ord_num * P * H * W)
...
class EfficientnetBackboneDrnnSddLossAspp(nn.Module):
    # denblock_config = (6, 12, 24, 48)
    def __init__(self, n_channels, reduction, k_ordinal_class, device, model_name, scale_factor=(14, 14), output_padding=0, pretrained=False):
        super(EfficientnetBackboneDrnnSddLossAspp, self).__init__()
        self.device = torch.device(device)
        # dense 1
        if pretrained:
            self.efficient_net = None # Not Implemented yet.
        else :
            self.efficient_net = EfficientNet.from_name(model_name, out_classes=1, in_channels=n_channels).to(self.device)

        n_channels = 1280


        self.scene_understanding_module = SceneUnderstandingModule(1280, scale_factor, device=self.device).to(self.device)

        self.transpose_conv1 = nn.ConvTranspose2d(n_channels, int(n_channels * reduction), kernel_size=2, stride=2, output_padding=output_padding).to(self.device)
        # # resblock
        self.resblock1 = ResidualBlock(752).to(self.device)

        n_channels = 1504
        # n_channels = int(n_channels * 6) # n_channels = 512
        self.transpose_conv2 = nn.ConvTranspose2d(n_channels, 120, kernel_size=2, stride=2).to(self.device)
        self.resblock2 = ResidualBlock(160).to(self.device)

        n_channels = 320
        self.transpose_conv3 = nn.ConvTranspose2d(n_channels, 40, kernel_size=2, stride=2).to(self.device)
        # n_channels = int(n_channels // 12)  # n_channels = 32
        self.resblock3 = ResidualBlock(64).to(self.device)
        self.transpose_conv4 = nn.ConvTranspose2d(128, 8, kernel_size=2, stride=2).to(self.device)
        self.resblock4 = ResidualBlock(24).to(self.device)

        self.ordinal_regression_conv1 = nn.Conv2d(48, k_ordinal_class * 2, kernel_size=1).to(self.device)
        self.orl1 = OrdinalRegressionLayer().to(self.device)
        self.last_conv = nn.Conv2d(48, 1, kernel_size=1).to(self.device)
    ...
    def forward(self, x):
        ...
        return (reg_output.to(self.device), self.orl1(ordinal_regression1))

def DrnnSddLossAspp(paramerters, load_pth=None):
    model = EfficientnetBackboneDrnnSddLossAspp(n_channels=paramerters['n_channels'],
                                                reduction=paramerters['reduction'],
                                                k_ordinal_class=paramerters['k_ordinal_class'],
                                                device=paramerters['device'],
                                                model_name=paramerters['model_name'],
                                                scale_factor=paramerters['scale_factor'],
                                                output_padding=paramerters['output_padding'])

    if load_pth is not None:
        print(f"{load_pth.split('/')[-2]}/{load_pth.split('/')[-1]} model is LOADED!!")
        path = load_pth
        state_dict = torch.load(path)
        for key in list(state_dict.keys()):
            new_key = key.replace('module.', '').replace('scene_understanding_', 'scene_understanding_module.', )
            #new_key = new_key.replace('module.efficient_net.', 'efficient_net.')
...
if __name__ == '__main__':
    input = torch.rand(1, 1, 72, 48) #N, C, H, W
    net = EfficientnetBackboneDrnnSddLossAspp(n_channels=1,
                                              reduction=0.5,
                                              k_ordinal_class=5,
                                              dev_0=torch.device("cpu"),
                                              dev_1=torch.device("cpu"),
                                              dev_2=torch.device("cpu"),
                                              dev_3=torch.device("cpu"),
