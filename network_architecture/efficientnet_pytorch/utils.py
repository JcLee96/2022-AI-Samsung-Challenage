...
class Conv3dDynamicSamePadding(nn.Conv3d):
    ...
    def forward(self, x):
        ih, iw = x.size()[-2:]
        kh, kw = self.weight.size()[-2:]
        if self.stride[1] is list and self.stride[2] is list:
            self.stride = (self.stride[0], self.stride[1][0], self.stride[2][0])
        sd, sh, sw = self.stride
        oh, ow = math.ceil(ih / sh), math.ceil(iw / sw) # change the output size according to stride ! ! !
        pad_h = max((oh - 1) * sh + (kh - 1) * self.dilation[1] + 1 - ih, 0)
        pad_w = max((ow - 1) * sw + (kw - 1) * self.dilation[2] + 1 - iw, 0)
        if pad_h > 0 or pad_w > 0:
            x = F.pad(x, [pad_w // 2, pad_w - pad_w // 2, pad_h // 2, pad_h - pad_h // 2])
...
class Conv3dStaticSamePadding(nn.Conv3d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, image_size=None, **kwargs):
        ...
        ih, iw = (image_size, image_size) if isinstance(image_size, int) else image_size
        kh, kw = self.weight.size()[-2:]
        if type(self.stride[1]) is list and type(self.stride[2]) is list:
            self.stride = (self.stride[0], self.stride[1][0], self.stride[2][0])
        sd, sh, sw = self.stride
        oh, ow = math.ceil(ih / sh), math.ceil(iw / sh)
        pad_h = max((oh - 1) * sh + (kh - 1) * self.dilation[1] + 1 - ih, 0)
        pad_w = max((ow - 1) * sw + (kw - 1) * self.dilation[2] + 1 - iw, 0)
        if pad_h > 0 or pad_w > 0:
            self.static_padding = nn.ZeroPad2d((pad_w // 2, pad_w - pad_w // 2,
...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, middle_channels, out_channels):
        ...
        self.up = nn.ConvTranspose3d(in_channels , middle_channels, kernel_size=(1, 2, 2), stride=(1, 2, 2))
